...
async def resolve_historique_client(_, info, client_id):
    ...
    # Trouver toutes les livraisons pour ces commandes
    livraisons = []
...
def resolve_route_optimale(_, info, port_depart, port_arrivee):
    with driver.session() as session:
        ...
        if result:
            # TODO: Parser le chemin retourné par Neo4J
            return {
...
async def resolve_creer_commande(_, info, input):
    ...
    # Calculer le nombre de caisses requises
...
def resolve_creer_route(_, info, port_depart, port_arrivee):
    # Calculer la distance entre les deux ports
    with driver.session() as session:
        # Récupérer les coordonnées des ports
